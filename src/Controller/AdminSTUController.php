  ...
  public function managerstu(Environment $twig, LoggerInterface $logger)
  {
    ...
    if(isset($scale_right) && ($scale_right > 4)){
        ...
        $allstu_query = " SELECT mu.id AS ID, UPPER(mu.username) AS USERNAME, uas.secret AS SECRET, CONCAT(CAST(uas.secret AS CHAR), UPPER(uas.username)) AS PAGE, "
                              ...
                              . " mu.phone1 AS PHONE, mu.address AS ADRESSE, mu.city AS QUARTIER, DATE_FORMAT(mu.datedenaissance, '%d/%m/%Y') AS DATEDENAISSANCE, UPPER(ifnull(mu.lieu_de_naissance, 'na')) AS LIEUDN, DATE_FORMAT(mu.create_date, '%d/%m/%Y') AS INSCDATE, ifnull(mu.compte_fb, 'na') AS FB, fEscapeStr(mu.etablissement_origine) AS ORIGINE, "
